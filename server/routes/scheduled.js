...
router.post('/', (req, res) => {
  const { name, amount, frequency, start_date, end_date } = req.body;
  ...
  const result = stmt.run(name, amount, normalizeFrequency(frequency), start_date, end_date || null);
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(
    `UPDATE scheduled_payments
     SET name = ?, amount = ?, frequency = ?, start_date = ?, end_date = ?, updated_at = datetime('now')
     WHERE id = ?`
  ).run(
    name ?? existing.name,
